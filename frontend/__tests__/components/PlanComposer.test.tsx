...
const defaults = {store:'TRADER_JOES' as const,days:7,servings:1,busy:false,onStoreChange:jest.fn(),onDaysChange:jest.fn(),onServingsChange:jest.fn(),onGenerate:jest.fn(),preferences:<p>Saved preferences</p>};
it('keeps labeled supported choices and only one generation action', () => {
  ...
  fireEvent.change(screen.getByLabelText('Store'),{target:{value:'WHOLE_FOODS'}});
  ...
  fireEvent.change(screen.getByLabelText('Servings'),{target:{value:'12'}});
...
it('preserves controlled values while generation disables edits and repeat submission', () => {
  render(<PlanComposer {...defaults} store="WHOLE_FOODS" days={3} servings={4} busy />);
