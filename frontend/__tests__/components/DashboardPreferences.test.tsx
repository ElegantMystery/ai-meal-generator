...
describe('dashboard preferences', () => {
  it('distinguishes loading and failure from unset preferences', () => {
    const { rerender } = render(<DashboardPreferences prefs={null} state="loading" />);
    ...
    expect(screen.getByText('Preferences unavailable. Check your settings before generating.')).toBeInTheDocument();
  ...
  it('shows saved preferences and a clearly scoped target beside the settings link', () => {
    render(<DashboardPreferences state="ready" prefs={{dietaryRestrictions:'high-protein', allergies:'peanuts; milk', targetCaloriesPerDay:2000}} />);
    expect(screen.getByText('High protein')).toBeInTheDocument();
    expect(screen.getByText('Allergies: peanuts, milk')).toBeInTheDocument();
    expect(screen.getByText('Target: 2,000 kcal / person / day')).toBeInTheDocument();
    expect(screen.getByRole('link', {name:'Edit preferences'})).toHaveAttribute('href', '/settings');
  });
});
