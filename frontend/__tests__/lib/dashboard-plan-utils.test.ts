import { featuredPlan, previewDays, initialDayIndex, localDateKey, type SavedPlan } from '@/lib/dashboard-plan-utils';

const plan = (id: number, startDate: string | null, endDate: string | null, createdAt: string | null): SavedPlan => ({ id, title: `Plan ${id}`, startDate, endDate, createdAt, planJson: null });
...
describe('dashboard plan selection', () => {
  ...
  it('does not mutate input or let invalid timestamps outrank known creation dates', () => {
    const plans = [plan(1, null, null, null), plan(2, null, null, '2026-09-17T12:00:00Z')];
...
describe('dashboard previews', () => {
  it('uses actual days, selects today when present, otherwise the first available day', () => {
    const days = previewDays(JSON.stringify({plan: [{date: '2026-09-16', meals: []}, {date: '2026-09-17', meals: []}, {date: '2026-09-19', meals: []}]}));
  ...
  it.each([null, '{bad', '{}', '{"plan":null}', '{"plan":[null,3,{}, {"date":"2026-02-30"}]}'])('safely handles malformed or absent JSON: %s', value => {
    expect(previewDays(value)).toEqual([]);
  });
  it('retains dated empty days and renders valid legacy flat item names defensively', () => {
    const days = previewDays(JSON.stringify({plan: [{date:'2026-09-17', meals:[null, {name:'Lunch', items:[null, {name:'Bread'}, {name:12}]}]}, {date:'2026-09-18', meals:null}]}));
    expect(days).toEqual([{date:'2026-09-17', meals:[{name:'Lunch', dishes:[], itemNames:['Bread']}]}, {date:'2026-09-18', meals:[]}]);
  });
  it('keeps real dish copy without inferring per-person calorie scope', () => {
    const days = previewDays(JSON.stringify({plan:[{date:'2026-09-17', meals:[{name:'Dinner', dishes:[null, {dishName:'Pasta', description:'With tomatoes', estimatedCalories:900}], items:[]}]}]}));
    expect(days[0].meals[0].dishes).toEqual([{name:'Pasta', description:'With tomatoes'}]);
  });
});
