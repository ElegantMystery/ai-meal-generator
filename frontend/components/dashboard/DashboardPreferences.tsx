...
export function DashboardPreferences({
  ...
  prefs: DashboardPreferencesData;
  ...
  return (
    <section aria-label="Saved preferences" className="space-y-3 text-sm">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="font-medium text-gray-700">Made for you</p>
        <Link
          href="/settings"
          className="inline-flex min-h-11 items-center gap-1.5 rounded-md text-brand-600 underline-offset-4 hover:underline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-brand-600"
        >
      ...
      {state === "loading" ? (
        ...
        <ul className="flex flex-wrap gap-2 text-brand-800">
          {[
            diet ? diet[0].toUpperCase() + diet.slice(1) : "Diet not set",
            allergies ? `Allergies: ${allergies}` : "Allergies not set",
            typeof target === "number" && Number.isFinite(target) && target > 0
              ? `Target: ${target.toLocaleString("en-US")} kcal / person / day`
              : "Calorie target not set",
          ].map((value) => (
            <li
              key={value}
              className="max-w-full break-words rounded-lg bg-brand-50 px-3 py-2"
            >
              {value}
            </li>
          ))}
