...
export function PlanComposer({
  ...
  return (
    <section
      id="new-plan-composer"
      aria-labelledby="composer-title"
      className="rounded-xl bg-brand-800 p-5 shadow-sm sm:p-7"
    >
      <p className="text-xs font-medium uppercase tracking-widest text-brand-200">
        Your grocery concierge
      </p>
      <h2 id="composer-title" className="mt-2 font-brand text-3xl text-white">
        Plan your next haul
      </h2>
      <p className="mt-2 text-sm leading-relaxed text-brand-100">
        Meals you’ll look forward to. A shopping list ready to go.
      </p>
      <form
        className="mt-6 space-y-4 rounded-xl bg-white p-4 sm:p-5"
        onSubmit={(event) => {
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-[2fr_1fr_1fr]">
          <div className="col-span-2 sm:col-span-1">
            <Select
              ...
              className="min-h-11"
              value={store}
              onChange={(event) => onStoreChange(event.target.value as Store)}
              disabled={busy}
            >
